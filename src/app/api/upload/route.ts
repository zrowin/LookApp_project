...
const MAX_STORED_IMAGE_DIMENSION = 500;

async function resizeImageForStorage(buffer: Buffer) {
  try {
    ...
    console.warn('Could not resize uploaded image, storing original buffer:', err);
    return buffer;
  }
}
...
export async function handleUpload({
  ...
  const storageBuffer = await resizeImageForStorage(buffer);
  ...
  async function resolveStorageUrl(path: string) {
    try {
      ...
      if (typeof from.createSignedUrl === 'function') {
        try {
          const { data: signedData, error: signErr } = await from.createSignedUrl(
            path,
            60 * 60 * 24 * 7
          );
  ...
  try {
    const thumbBuffer = await createThumbnail(storageBuffer, 400);
    ...
    const { error: thumbErr } = await supabaseServer.storage
      .from('clothing-images')
      .upload(thumbPath, thumbBuffer, { upsert: false, contentType: 'image/webp' });
    if (!thumbErr) {
  ...
  console.log('Upload result for', imageId, {
    url: publicUrl,
