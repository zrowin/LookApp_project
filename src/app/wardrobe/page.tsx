...
import {
  getShelves,
  saveShelves,
  addShelf as addShelfUtil,
  updateShelfName,
  deleteShelf as deleteShelfUtil,
} from '@/lib/shelves';
...
export default function WardrobePage() {
  ...
  function addShelf(name?: string) {
    const title =
      ...
      (typeof window !== 'undefined'
        ? (window.prompt('Nazwa półki') ?? 'Nowa półka')
        : 'Nowa półka');
  ...
  function renameShelf(id: string) {
    ...
    const newName =
      typeof window !== 'undefined' ? window.prompt('Nowa nazwa', current?.name ?? '') : '';
    if (newName) {
